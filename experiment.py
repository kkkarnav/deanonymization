...
def auxillary_columns(df, r, count):
    ...
    if args.aux == "mostinfo":
        ...
        # population = [col for col in df.columns[362:] if col in support_columns(df, r)]
...
def auxillary(df, r, count):
    ...
    cols = auxillary_columns(df, r, count)
    all_aux.extend(cols)
...
def scoreboard(df, count):
    ...
    for r1 in tqdm(list(df.index)):
        scores.loc[r1, :] = pd.Series(df.index).progress_apply(lambda r2: score(df, aux_series.loc[r1, ~aux_series.loc[r1].isna()], r2))
...
results.to_csv(f"D:/code/results_{args.feature_count}features.csv", index=False)
...
all_aux.to_csv(f"D:/code/aux_{args.feature_count}features.csv", index=False)
all_aux_types.to_csv(f"D:/code/aux_types_{args.feature_count}features.csv", index=False)
