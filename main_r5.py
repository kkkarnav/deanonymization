...
print(args.robust)
print(type(args.robust))
whether_robust = "r" if args.robust else ""
print(whether_robust)

if args.robust:
    print("yes")
    results = scoreboard_robust(df4, args.feature_count)
    results.to_csv(f"{BASE_PATH}/results/r5r/resultscount{args.feature_count}_robust.csv", index=False)
else:
    print("no")
    results = scoreboard(df4, args.feature_count)
    results.to_csv(f"{BASE_PATH}/results/r5/resultscount{args.feature_count}.csv", index=False)
