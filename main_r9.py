...
if args.robust:
    ...
    results.to_csv(f"{BASE_PATH}/results/r9r/resultscount{args.feature_count}_robust.csv", index=False)
else:
    results = scoreboard(df4, args.feature_count)
    results.to_csv(f"{BASE_PATH}/results/r9/resultscount{args.feature_count}.csv", index=False)
